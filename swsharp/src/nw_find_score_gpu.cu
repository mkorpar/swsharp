...
__device__ static void solveShortDelegated(int d, VBus vBus, int2* hBus, Sub sub) { 
    ...
    for (int i = 0; i < blockDim.x; ++i) {
        ...
        if (0 <= row && row < rows_) {
            ...
            if (atom.rScr.x == score_) { res_.x = row; res_.y = col; }
            else if (atom.rScr.y == score_) { res_.x = row + 1; res_.y = col; }
            else if (atom.rScr.z == score_) { res_.x = row + 2; res_.y = col; }
            else if (atom.rScr.w == score_) { res_.x = row + 3; res_.y = col; }
